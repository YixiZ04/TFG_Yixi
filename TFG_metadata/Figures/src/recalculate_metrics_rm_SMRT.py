...
def _get_no_smrt_folds (seed_path2folds, path2all_results):
    ...
    for i in range(10):
        ...
        print(len(no_smrt_df))
        unique_ids = no_smrt_df["molecule_id"].unique()
        result_df = all_res_df[all_res_df["molecule_id"].isin(unique_ids)]

        final_array.append(result_df)
    return final_array

def _get_metrics_df (folds_array, path2res):
    metrics_dict = {
        "MAE": [],
        "RMSE": [],
        "MRE": [],
    ...
    pd.DataFrame(metrics_dict).to_csv(os.path.join(path2res, "new_result_summary.tsv"), sep="\t", index=False)
    return metrics_dict
...
def main():
    ...
    fold_type = ["cc_split", "ms_split", "random_split"]
    split_type = ["cc_split", "scaffold_split", "random_split"]
    moldesc_type = ["RepoRT_RP_kfold_moldesc", "RepoRT_RP_kfold"]

    for fold, split in zip(fold_type, split_type):
        path2entries = os.path.join(project_dir,
                                       "data",
                                       "RepoRT_RP",
                                       "processed_data",
                                       "with_SMRT",
                                       "kfolds",
                                       fold)
        for moldesc in moldesc_type:
            seed_path = os.path.join(project_dir, "results4TFG", moldesc, "with_SMRT", "no_filtered",
                                     split, "01_06_2026/")
            path2all_res = os.path.join(seed_path, "all_results.tsv")
            no_smrt_folds_array = _get_no_smrt_folds(path2entries, path2all_res)
            metrics_dict = _get_metrics_df(no_smrt_folds_array, seed_path)
